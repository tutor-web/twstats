...
qn_tables <- c(
    'eurostat/tin00073/BE',
    'eurostat/tin00073/BG',
    'cbso/82610ENG/total')
...
question <- function(permutation, data_frames) {
    ...
    qtext = paste0(
        ...
        'Find the difference of ', tbl$unit, ' between ', min(d$year), ' and ', max(d$year),"\n\n",
        paste(capture.output(print.data.frame(head(d))), collapse = "\n"))
    choices = c(
        d[d$year == max(d$year), 'value'] - d[d$year == min(d$year), 'value'],
        d[d$year == max(d$year), 'value'] - d[d$year == min(d$year), 'value'] - 10,
        (d[d$year == max(d$year), 'value'] - d[d$year == min(d$year), 'value']) / 10)
